...
public class DocumentFileRestTest {
  ...
  public void testGetDocumentFolder() throws Exception {
    ...
    PermissionEntryEntity permissionEntryEntity1 =
                                                 new PermissionEntryEntity(EntityBuilder.toIdentityEntity(identityManager,
                                                                                                          spaceService,
                                                                                                          Long.valueOf(userIdentity.getId())),
                                                                           "set_property");
    permissionEntryEntity.setPermission("add_node");
    permissionEntryEntity.setIdentity(EntityBuilder.toIdentityEntity(identityManager,
                                                                     spaceService,
                                                                     Long.valueOf(userIdentity.getId())));
  ...
  public void testGetBreadCrumbs() throws Exception {
    ...
    Response response1 = documentFileRest.getBreadcrumb(null, null, "");
    assertEquals(Response.Status.BAD_REQUEST.getStatusCode(), response1.getStatus());
  ...
  public void testCreateFolder() throws Exception {
    ...
    when(documentFileRest.moveDocument("11111111",
                                       2L,
                                       "/Groups/spaces/test/Documents/test",
                                       null)).thenThrow(RuntimeException.class);
  ...
  public void testUpdatePermissions() throws Exception {
    ...
    assertEquals(Response.Status.UNAUTHORIZED.getStatusCode(), response3.getStatus());
    // assert forbidden response status when the share documents forbidden by
    // administrators
  ...
  public void testCreateShortcut() throws Exception {
    ...
    when(documentFileRest.createShortcut("11111111",
                                         "/Groups/spaces/test/Documents/test",
                                         null)).thenThrow(RuntimeException.class);
    response = documentFileRest.createShortcut("11111111", "/Groups/spaces/test/Documents/test", null);
    assertEquals(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(), response.getStatus());
  ...
  public void moveDocuments() throws IllegalAccessException {
    ...
    response = documentFileRest1.moveDocuments(1, 1L, documents, "destPath");
    assertEquals(Response.Status.OK.getStatusCode(), response.getStatus());
    doThrow(new RuntimeException()).when(documentFileService1)
                                   .moveDocuments(anyInt(), anyLong(), anyList(), anyString(), anyLong());
    response = documentFileRest1.moveDocuments(1, 1L, documents, "destPath");
    assertEquals(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(), response.getStatus());
  }
  ...
  public void downloadDocument() {
    ...
    Response response = documentFileRest.downloadDocument(null, null);
    assertEquals(Response.Status.BAD_REQUEST.getStatusCode(), response.getStatus());
    when(publicDocumentAccessService.isPublicDocumentAccessExpired("123")).thenReturn(true, false);
    when(publicDocumentAccessService.isDocumentPublicAccessValid("123", null)).thenReturn(false, true);
    ...
    response = documentFileRest.downloadDocument("123", null);
    assertEquals(Response.Status.OK.getStatusCode(), response.getStatus());
    when(externalDownloadService.getDocumentDownloadItem("123")).thenThrow(RuntimeException.class);
    response = documentFileRest.downloadDocument("123", null);
    assertEquals(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(), response.getStatus());
  ...
  public void testGetDocumentsThumb() throws Exception {
    ...
    Response response = documentFileRest.getDocumentThumb(uriInfo, request, null, "documentsImage", null, null);
    assertEquals(400, response.getStatus());
    documentFileRest.getDocumentThumb(uriInfo, request, "docId", null, null, null);
    assertEquals(400, response.getStatus());
    ...
    org.exoplatform.services.security.Identity root = new org.exoplatform.services.security.Identity(userName);
    ConversationState.setCurrent(new ConversationState(root));
    when(imageThumbnailService.getOrCreateThumbnail("docId", "documentsImage", userName, 250, 250)).thenReturn(fileThumbnail);
    response = documentFileRest.getDocumentThumb(uriInfo, request, "docId", "documentsImage", null, "250x250");
    assertEquals(200, response.getStatus());
  }
}
