  ...
  <div class="d-flex flex-nowrap align-center">
    ...
    <div class="position-relative overflow-hidden">
      ...
      <v-btn
        ...
        {{ $t('documentsWidget.upload') }}
      </v-btn>
    </div>
    <v-btn
      v-if="!sending && !isDefault"
      :title="$t('documentsWidget.resetDefault')"
      class="ms-2"
      icon
      @click="reset">
...
export default {
  ...
  computed: {
    ...
    isDefault() {
      return !this.documentsWidget?.iconSrc && !this.documentsWidget?.iconUrl;
    },
  },
  methods: {
    reset() {
    ...
    uploadFile(file) {
      ...
      if (file && file.size) {
        if (file.size > this.maxFileSize) {
          this.$root.$emit('alert-message', this.$t('documentsWidget.tooBigFile.label'), 'error');
          return;
        }
        this.sending = true;
        const self = this;
        return this.$uploadService.upload(file)
          .then(uploadId => {
            if (uploadId) {
              ...
              this.$root.$emit('alert-message', this.$t('documentsWidget.uploadingError'), 'error');
            }
          })
