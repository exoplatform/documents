...
<template>
  <div
    v-if="hasDocuments && (showHeader || canEdit)"
    :class="{
    ...
    <v-spacer />
    <div
      v-if="hasDocuments && (seeMoreUrl || hoverEdit)"
      ...
      <v-fab-transition hide-on-leave>
        <v-btn
          v-if="hoverEdit"
          :title="$t('documents.label.editSettings')"
          :class="{
  ...
  <div v-else-if="!hasDocuments && canEdit" class="d-flex align-center justify-center">
    <v-btn
      class="primary"
      elevation="0"
      outlined
      ...
      {{ $t('documents.label.addDocumentsButton') }}
    </v-btn>
  </div>
</template>
<script>
export default {
  ...
  computed: {
    ...
    header() {
      return this.documentsWidgetSettings?.header?.[this.$root.language] || this.documentsWidgetSettings?.header?.[this.$root.defaultLanguage];
    },
    seeMoreUrl() {
      return this.$documentsWidgetService.toLinkUrl(this.documentsWidgetSettings?.seeMore);
    },
  },
};
</script>
